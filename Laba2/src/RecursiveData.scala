...
object RecursiveData {
  ...
  // Да, для этого нужно определить Nil следующим образом: case class Nil[A](head: A) extends List[A]
  ...
  def main(args: Array[String]): Unit = {
    println("listIntEmpty с не пустой списком: " +
      testListIntEmpty(Cons(1,Cons(2,Cons(3,Cons(4,Cons(5,Nil())))))))

    println("listIntEmpty с пустым списком: " + testListIntEmpty(Nil()))

    println("\nlistIntHead с не пустым списком: " +
      testListIntHead(Cons(1,Cons(2,Cons(3,Cons(4,Cons(5,Nil())))))))

    println("listIntHead с пустым списком: " + testListIntHead(Nil()))

    println("\nДерево: " + Node(Node(Leaf(1), Leaf(2)), Leaf(3)))
  }
}
